...
const EvaluationSubmitButton = memo(() => {
        ...
        handleSubmit(async data => {
            try {
                ...
                if (validation !== true) {
                    ...
                    showToast('Alguns campos obrigatórios não foram preenchidos ou contêm dados inválidos.', 'error', {
                ...
                showToast('Ocorreu um erro inesperado ao processar suas informações. Verifique sua conexão e tente novamente.', 'error', {
    ...
    return (
        <Button variant="primary" size="md" disabled={!isValid || isSubmitting} onClick={onSubmit} className={`transition-all duration-200 ${!isValid || isSubmitting ? 'bg-primary-200 text-primary-400 cursor-not-allowed hover:bg-primary-200' : 'bg-primary-500 text-white hover:bg-primary-600 cursor-pointer'}`}>
