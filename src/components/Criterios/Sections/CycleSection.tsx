...
export function CycleSection() {
    ...
    const handleStartCycle = async (endDate: string) => {
        ...
        showToast(
            `O ciclo foi iniciado com sucesso! Data de término: ${formatDate(endDate)}`,
            'success',
    ...
    const handleCancelCycle = async () => {
        ...
        showToast(
            'Ciclo cancelado com sucesso! As avaliações foram apagadas.',
            'success',
    ...
    const canStartCycle =
        !!currentCycle && !currentCycle.isOpen && currentCycle.allTracksSet;

    return (
        <div className="flex flex-col gap-8 sm:gap-6 px-2 sm:px-0">
            {!currentCycle?.allTracksSet && (
                <AlertMessage
                    message="É necessário definir os critérios de todas as trilhas antes de iniciar o ciclo."
                    type="alert"
                />
            )}
            <section className="pb-8 sm:pb-6 border-b border-gray-300 mb-4 sm:mb-2">
                ...
                <CycleCard
                    label={cycleLabel}
                    showButton={!!currentCycle && !currentCycle.isOpen}
                    canStart={canStartCycle}
                    status={currentCycle?.isOpen ? 'aberto' : undefined}
                ...
                <div className="max-h-96 md:max-h-96 lg:max-h-96 xl:max-h-96 2xl:max-h-96 sm:max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                    {mockCycles
                        .filter(
                            c =>
                                c.cycleName !==
                                cycleLabel.replace('Ciclo ', ''),
                        )
                        .map(cycle => (
                            <CycleCard
                                key={cycle.cycleName}
                                label={`Ciclo ${cycle.cycleName}`}
                                status="finalizado"
                            />
                        ))}
                </div>
            ...
            <ExtendCycleModal
                ...
                currentEndDate={currentCycle?.dataFim || ''}
                year={year}
                semester={semester}
            />
        </div>
