            ...
            <div className="flex-1 flex flex-col max-h-full overflow-y-auto overflow-x-hidden custom-scrollbar space-y-3 pr-2">
                {(status === 'upcoming' || status === 'undefined') ? (
                    ...
                    evaluationsSummary.map(summary => {
                        const currentCycleScore = summary.scores?.find(s => String(s.cycleName) === String(currentCycle.name));
                        const autoEvaluationScore = currentCycleScore?.autoEvaluationScore;
                        return (
                                    ...
                                    <div className="min-w-0 flex-1">
                                        <div className="flex items-center gap-2 sm:block">
                                            ...
                                            <div className="sm:hidden">
                                                <Badge
                                                    label={typeof autoEvaluationScore === 'number' ? 'Finalizado' : 'Pendente'}
                                                    variant={typeof autoEvaluationScore === 'number' ? 'success' : 'warning'}
                                                    size="sm"
                                                />
                                            </div>
                                        </div>
