    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-6">
            <CardContainer>
                ...
                {users.length === 0 ? (
                    <div className="text-gray-400 text-center py-8">Nenhum usuário encontrado</div>
                ) : (
                    <div className="space-y-0 max-h-[400px] overflow-y-auto pr-2">
                        {users.map((user, idx) => (
                            <div key={user.email} className={`flex items-center justify-between gap-4 ${idx !== 0 ? 'py-4' : 'pb-4'} ${idx !== users.length - 1 ? 'border-b-2 border-b-gray-300' : ''}`}>
                                <CollaboratorCard collaborator={{ name: user.name, position: user.position ?? '' }} />
            ...
            <CardContainer>
                ...
                {projects.length === 0 ? (
                    <div className="text-gray-400 text-center py-8">Nenhum projeto encontrado</div>
                ) : (
                    <div className="space-y-0 max-h-[400px] overflow-y-auto pr-2">
                                                    ...
                                                    <div className="flex flex-col gap-1">
                                                        ...
                                                        <div className="flex flex-row flex-wrap gap-5 pb-1">
                                                            <div className="flex flex-col items-start max-w-xs">
                                                                <CollaboratorCard collaborator={{ name: managerUser.name, position: managerUser.position ?? '' }} variant="compact" />
                                                                ...
                                                                project.leaders.map((leader) => {
                                                                    ...
                                                                    return leaderUser ? (
                                                                        <div key={leader.email} className="flex flex-col items-start max-w-xs">
                                                                            <CollaboratorCard collaborator={{ name: leaderUser.name, position: leaderUser.position ?? '' }} variant="compact" />
                                                                ...
                                                                project.collaborators.map((collab) => {
                                                                    ...
                                                                    return collabUser ? (
                                                                        <div key={collab.email} className="flex flex-col items-start max-w-xs">
                                                                            <CollaboratorCard collaborator={{ name: collabUser.name, position: collabUser.position ?? '' }} variant="compact" />
