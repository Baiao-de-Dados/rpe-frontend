...
export default function AddCriterionModal({ open, onClose, onAdd }: AddCriterionModalProps) {
    ...
    return (
        <Modal open={open} onClose={handleClose} className="max-w-5xl w-full">
            <form onSubmit={form.handleSubmit(handleSubmit)} className="p-4 w-full max-w-5xl mx-auto">
                ...
                <div className="flex flex-col gap-4">
                    <InputWithTitle
                        ...
                        labelPosition="top"
                    />
                    <TextAreaWithTitle title="Descrição" placeholder="Descrição do critério" value={form.watch('description') || ''} onChange={e => form.setValue('description', e.target.value)} error={form.formState.errors.description?.message} maxLength={500} />
                </div>
